...
behavior MinerThread (i_receiver c_header, IProtocolMaster miner_bus,
  out unsigned int nonce)
{
  ...
  void main (void)
  {
    ...
    while (true)
    {
      ...
      miner_bus.masterWrite(C2, &header, sizeof(header));

      // Wait for the hardware miner to return a status
      miner_bus.masterRead(C3, &hw_return, sizeof(hw_return));
      if (hw_return.status)
      {
...
behavior Controller(i_sender c_request, i_receiver c_response,
  i_sender c_header, IProtocolMaster miner_bus, in unsigned int nonce)
{
  ...
  void main (void)
  {
    ...
    while (true)
    {
      ...
      if (block_count > old_block_count)
      {
        ...
        miner_bus.masterWrite(C1, &abort_command, sizeof(abort_command));
...
behavior MiningSW (i_sender c_request, i_receiver c_response,
  IProtocolMaster miner_bus)
{

  c_double_handshake c_header;
  unsigned int nonce = 0;

  MinerThread miner_thread(c_header, miner_bus, nonce);
  Controller controller(c_request, c_response, c_header, miner_bus, nonce);
