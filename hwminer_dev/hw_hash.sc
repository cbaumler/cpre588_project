...
behavior Process_Block_Header(i_receiver  c_blk_hdr, out event e_tstart,
  out event e_tstop, i_sender c_nonce, i_sender c_perf) {
  ...
  const unsigned int sha256_init_state[8] = {
  	0x6a09e667, 0x0bb67ae85, 0x3c6ef372, 0xa54ff53a,
  ...
  void hashmeter(unsigned int time, unsigned long hashes_done)
	{
  ...
  bool scanhash(unsigned char *midstate,
	...
	// Outputs:
	//-----------------------------------------------------------------------
  bool hash_it_to_pieces(Block_Header *blk_hdr) {
  ...
  rc = scanhash(work.midstate,
	...
	void main(void) {
		...
		Block_Header blk_hdr;
		ReturnNonce return_nonce;
		int stats; // TODO: Get real stats
      ...
      c_blk_hdr.receive(&blk_hdr, sizeof(blk_hdr));
      ...
      return_nonce.status = hash_it_to_pieces(&blk_hdr);
      notify(e_tstop);  // Stop watchdog

      // Set the final nonce value into the return nonce structure
      return_nonce.nonce  = blk_hdr.nonce;

      // Report hash results and performance statistics
      c_nonce.send(&return_nonce, sizeof(return_nonce));
      c_perf.send(&stats, sizeof(stats));
...
behavior HW_Hash(i_receiver  c_blk_hdr,
  ...
  int command;
	...
	event e_tstart;
	event e_tstop;   // TODO: Wasn't 100% sure what the plan was for these

  Process_Block_Header process_block_header(c_blk_hdr, e_tstart, e_tstop,
	  c_nonce, c_perf);
